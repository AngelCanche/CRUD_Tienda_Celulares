...
// 1. Recepción y limpieza de datos (Forzamos a entero para evitar errores de tipo SQL)
...
try {
    if ($id !== null && $id > 0) {
        ...
        $sql = "UPDATE modelos SET nombre = :nombre, id_marca = :id_marca WHERE id = :id";
        $stmt = $db->prepare($sql);
        $stmt->bindParam(':id', $id, PDO::PARAM_INT);
    } else {
        // LÓGICA DE INSERCIÓN
        $sql = "INSERT INTO modelos (nombre, id_marca) VALUES (:nombre, :id_marca)";
        $stmt = $db->prepare($sql);
    }
    ...
    <div class='success-card'>
        ...
        <div class='success-text'>Modelo " . ($id ? "actualizado" : "guardado") . " correctamente</div>
    ...
    <style>
        ...
        .error-card { background: rgba(0,0,0,0.3); backdrop-filter: blur(20px); border: 1px solid rgba(239, 68, 68, 0.3); padding: 40px; border-radius: 24px; text-align: center; max-width: 500px; box-shadow: 0 25px 50px rgba(0,0,0,0.7); }
        .error-icon { font-size: 4rem; color: #ef4444; margin-bottom: 20px; display: block; }
        .error-message { color: #fca5a5; background: rgba(0,0,0,0.3); padding: 15px; border-radius: 12px; border-left: 4px solid #ef4444; text-align: left; }
        .btn-retry { margin-top: 30px; display: inline-block; padding: 14px 30px; border: 1px solid #ef4444; color: white; text-decoration: none; border-radius: 12px; font-weight: 600; }
    </style>
